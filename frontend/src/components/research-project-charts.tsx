...
import { fetchProjectResearchsOptions } from "@/lib/api";

export function ResearchProjectCharts({ projectId }: { projectId: string }) {
  const { data } = useQuery(fetchProjectResearchsOptions(projectId));
...
function ListChart({
  ...
  researchs: Record<string, { name: string; createdAt: string }>;
